...
export class EventService {
  ...
  protected convertDateFromClient(event: IEvent): IEvent {
    const copy: IEvent = Object.assign({}, event, {
      creationDate: event.creationDate != null && event.creationDate.isValid() ? event.creationDate.format(DATE_FORMAT) : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((event: IEvent) => {
        event.creationDate = event.creationDate != null ? moment(event.creationDate) : null;
      });
    }
    return res;
  }
